...
def test_handle_url():
    ...
    nbef = printer.handle_url(config["fornof.gifts.url"],maml_phrase_item['phrases'][0])
...
def test_handle_file():
    ...
    nbef = printer.handle_file(config["fornof.mamlsocket"],maml_phrase_item['phrases'][2])
    print(nbef, 'nbef')
    assert nbef['notes'] != None
...
def test_handle_maml_phrases_file():
    ...
    nbef = printer.handle_file(config["fornof.mamlsocket"],maml_phrase_item['phrases'][1])
    assert nbef['notes'] != None
...
#@pytest.mark.skip(reason="really_slow, slows down tests, but works if unskipped")
